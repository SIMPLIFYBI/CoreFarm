...
export default function ConstructionTrackPreview({ plannedDepth, actualDepth, intervals, constructionById }) {
  const uid = useId();
  ...
  const normalized = useMemo(() => {
    return (intervals || [])
      .map((r) => {
        ...
        const construction_type_id = r.construction_type_id || "";
        if (!Number.isFinite(from) || !Number.isFinite(to)) return null;
        if (!(from < to)) return null;
        if (!construction_type_id) return null;
        return {
          id: r.id || null,
          from,
          to,
  ...
  const hatchId = `conHatch-${uid}`;
  const clipId = `conTrackClip-${uid}`;
    ...
    <div className="rounded-xl border border-white/10 bg-white/[0.02] p-3">
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} className="block">
        ...
        <g clipPath={`url(#${clipId})`}>
          {(normalized || []).map((it, i) => {
            const t = it.construction_type_id ? constructionById?.get?.(it.construction_type_id) : null;
            ...
            return (
              <g key={it.id || `${it.construction_type_id}-${it.from}-${it.to}-${i}`}>
                <rect
                  x={trackX + 2}
                  y={y1}
                  width={trackW - 4}
          ...
          {hasPlanned && hasActual && actual > planned && (
            <rect x={trackX} y={plannedY} width={trackW} height={Math.max(0, actualY - plannedY)} fill={`url(#${hatchId})`} />
          )}
        </g>
        ...
        {normalized.length === 0 && (
          <text x={W / 2} y={H / 2} textAnchor="middle" fontSize="12" fill="rgba(148,163,184,0.9)">
            No construction intervals
          </text>
        )}
      </svg>

      <div className="mt-2 text-[11px] text-slate-400">
        Tip: add intervals in the <b>Construction</b> tab, then they’ll appear here.
      </div>
    </div>
  );
}