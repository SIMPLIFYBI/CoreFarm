      ...
      {!selectedHole ? (
        ...
        <div className="card p-3 space-y-4">
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3 space-y-3">
            ...
            <div className="text-[11px] text-slate-500">These are the same collar coordinates used by the map for plotting hole locations.</div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <label className="text-xs text-slate-300">
                Collar longitude
                <input
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3 space-y-3">
            <div className="text-xs text-slate-300 font-medium">Collar (Projected)</div>
            <div className="text-[11px] text-slate-500">Use projected coordinates when the hole belongs to a project with a configured coordinate system.</div>
            {projectCrsLabel ? <div className="text-[11px] text-cyan-200/80">Working CRS: {projectCrsLabel}</div> : null}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <label className="text-xs text-slate-300">
                Collar easting
                <input
                  ...
                  type="number"
                  step="0.001"
                  placeholder="Projected easting"
                  value={collarEastingInput ?? ""}
                  disabled={!canEditHole}
                  onChange={(e) => onAttributesChange?.("collar_easting", e.target.value)}
                />
              </label>
              <label className="text-xs text-slate-300">
                Collar northing
                <input
                  ...
                  type="number"
                  step="0.001"
                  placeholder="Projected northing"
                  value={collarNorthingInput ?? ""}
                  disabled={!canEditHole}
                  onChange={(e) => onAttributesChange?.("collar_northing", e.target.value)}
                />
              </label>
              <label className="text-xs text-slate-300">
                Elevation (m)
