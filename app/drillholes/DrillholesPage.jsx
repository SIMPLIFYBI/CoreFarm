...
export default function DrillholesPage() {
  ...
  }, [selectedId, supabase, TASK_TYPES]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-8">
      <h1 className="text-2xl font-semibold">Drillholes / Tasks</h1>
      ...
      <section className="space-y-3 hidden md:block">
        ...
        <p className="text-sm text-gray-600">
          Paste CSV/TSV from Excel with headers: hole_id, depth, planned_depth, drilling_diameter
        </p>
        <div>
          <button type="button" onClick={() => { setShowBulk(true); setParsed([]); }} className="btn btn-primary">
            Open bulk uploader
          </button>
        </div>
      </section>
                        ...
                        <td colSpan={8} className="p-0">
                          <div className="p-4 space-y-4 border-t">
                            <div className="flex items-center justify-between">
                              ...
                              <button type="button" className="btn btn-xs" onClick={saveIntervals} disabled={!selectedId}>
                                Save Tasks
                              </button>
                            </div>
                                  ...
                                  <div className="space-y-2">
                                    {(intervals[t] || []).map((row, idx) => (
                                      <div key={idx} className="flex items-center gap-2">
                                        ...
                                        />
                                        <button type="button" className="btn btn-xs" onClick={() => removeInterval(t, idx)}>
                                          ×
                                        </button>
                                      </div>
                                    ))}
